...
export class Store {
  ...
  async openDevEuis(kind: string): Promise<Set<string>> {
    const rows = await this.query<{ dev_eui: string }>(
      `SELECT dev_eui FROM leadsman.alert WHERE kind = $1 AND resolved_at IS NULL`,
      [kind],
    );
  ...
  > {
    return this.query(
      `SELECT kind, dev_eui, device_name, severity, summary, raised_at, notified_at
