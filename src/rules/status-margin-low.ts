...
const rule: Rule = {
  ...
  description:
    "Flags devices whose MAC-layer demodulation margin (event_status, from DevStatusAns) " +
  ...
  async run(ctx) {
    const minMargin = num(ctx.params, 'minMarginDb');
    const criticalMargin =
      ctx.params.criticalMarginDb === null || ctx.params.criticalMarginDb === undefined
        ? null
        : num(ctx.params, 'criticalMarginDb');
    const lookbackHours = int(ctx.params, 'lookbackHours');
    const minReadings = int(ctx.params, 'minReadings');

    if (criticalMargin !== null && criticalMargin > minMargin) {
      throw new Error(
        `criticalMarginDb (${criticalMargin}) must be <= minMarginDb (${minMargin})`,
      );
    }
    ...
    const rows = await ctx.query<Row>(
      ...
      [lookbackHours, minReadings, minMargin, ...sc.values],
    );
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        summary:
          `${name} downlink margin ${avg}dB over ${lookbackHours}h ` +
