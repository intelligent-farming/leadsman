...
const rule = {
    id: 'join-churn',
    description: 'Flags devices that rejoined the network more than a few times in the window, or ' +
    ...
    async run(ctx) {
        ...
        const maxJoins = (0, params_1.int)(ctx.params, 'maxJoins');
        const minUplinksForRatio = (0, params_1.int)(ctx.params, 'minUplinksForRatio');
        const ratioRaw = ctx.params.maxJoinsPerUplinkRatio;
        const maxRatio = ratioRaw === null || ratioRaw === undefined ? null : (0, params_1.num)(ctx.params, 'maxJoinsPerUplinkRatio');
        if (maxJoins < 1)
            throw new Error('maxJoins must be at least 1');
        ...
        const sc = (0, scope_1.scopeClause)((0, scope_1.resolveScope)(ctx.params), 5);
        ...
        const rows = await ctx.query(`SELECT j.dev_eui,
       ...
       HAVING count(*) > $2::int
        ...
        ORDER BY count(*) DESC`, [lookbackHours, maxJoins, maxRatio, minUplinksForRatio, ...sc.values]);
        return rows.map((row) => {
            ...
            return {
                ...
                summary: `${name} rejoined ${joins}× in ${lookbackHours}h (${addrNote}, ` +
                ...
                detail: {
                    ...
                    joinsPerUplink: ratio === null ? null : (0, params_1.round)(ratio, 3),
                    maxJoins,
...
//# sourceMappingURL=join-churn.js.map