...
const rule = {
    id: 'decode-failure',
    description: 'Flags devices whose uplinks arrive with a NULL, JSON-null, or empty decoded ' +
    ...
    async run(ctx) {
        ...
        const minFailures = (0, params_1.int)(ctx.params, 'minFailures');
        const minFailureRatio = (0, params_1.num)(ctx.params, 'minFailureRatio');
       ...
       HAVING count(*) FILTER (WHERE object IS NULL OR jsonb_typeof(object) = 'null' OR object = '{}'::jsonb) >= $2::int
        ...
        ORDER BY failed DESC`, [lookbackHours, minFailures, minFailureRatio, ...sc.values]);
        return rows.map((row) => {
            ...
            return {
                ...
                summary: `${name}${profile}: ${failed} of ${total} uplinks did not decode ` +
                    `(${(0, params_1.round)(ratio * 100, 0)}%) — check the payload codec`,
                ...
                detail: {
                    ...
                    totalUplinks: total,
                    failureRatio: (0, params_1.round)(ratio, 3),
...
//# sourceMappingURL=decode-failure.js.map