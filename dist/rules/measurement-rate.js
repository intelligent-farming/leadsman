...
const rule = {
    id: 'measurement-rate',
    description: 'Flags devices whose reading is changing faster than a per-hour limit, computed as ' +
    ...
    async run(ctx) {
        ...
        for (const s of stats) {
            ...
            const breached = direction === 'falling'
                ? rate < -maxRate
                : direction === 'rising'
                    ? rate > maxRate
            ...
            findings.push({
                ...
                summary: `${name} ${s.matchedPath} ${verb} ${(0, params_1.round)(Math.abs(rate), 2)}${unit}/h ` +
                    `(${(0, params_1.round)(s.first, 1)} → ${(0, params_1.round)(s.last, 1)}${unit} over ${(0, params_1.round)(spanHours, 1)}h, ` +
                    `limit ${maxRate}${unit}/h)`,
                detail: {
                    measurement: s.matchedPath,
                    ratePerHour: (0, params_1.round)(rate, 3),
                    maxRatePerHour: maxRate,
                    direction,
                    unit: unit || null,
                    firstValue: (0, params_1.round)(s.first, 3),
                    lastValue: (0, params_1.round)(s.last, 3),
                    delta: (0, params_1.round)(delta, 3),
...
//# sourceMappingURL=measurement-rate.js.map