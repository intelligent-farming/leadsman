...
const rule = {
    ...
    description: 'Flags devices that reported a measurement earlier in the window but have stopped, ' +
    ...
    async run(ctx) {
        ...
        const recentHours = (0, params_1.int)(ctx.params, 'recentHours');
        const minRecentUplinks = (0, params_1.int)(ctx.params, 'minRecentUplinks');
        const minHistorical = (0, params_1.int)(ctx.params, 'minHistoricalSightings');
        if (recentHours >= lookbackHours) {
            throw new Error(`recentHours (${recentHours}) must be less than lookbackHours (${lookbackHours}) — ` +
                'the check compares a recent window against a longer history');
        }
        const scope = (0, scope_1.resolveScope)(ctx.params);
        const presence = await (0, measurement_1.pathPresence)(ctx, paths, lookbackHours, recentHours, scope);
        const findings = [];
        for (const p of presence) {
            // Only devices with an established history of reporting this field.
            if (p.everSeen < minHistorical)
                continue;
            // Only devices we know are still talking — otherwise this is device-silent's job.
            if (p.recentUplinks < minRecentUplinks)
                continue;
            ...
            findings.push({
                ...
                summary: `${name} stopped reporting ${p.matchedPath}: ${p.recentUplinks} uplinks in the ` +
...
//# sourceMappingURL=measurement-missing.js.map