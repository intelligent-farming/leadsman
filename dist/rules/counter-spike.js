...
const rule = {
    id: 'counter-spike',
    description: 'Flags monotonic totals (water, energy, pulses) advancing faster than a per-hour ' +
    ...
    async run(ctx) {
        ...
        const maxRate = (0, params_1.num)(ctx.params, 'maxRatePerHour');
        const lookbackHours = (0, params_1.int)(ctx.params, 'lookbackHours');
        const minSamples = (0, params_1.int)(ctx.params, 'minSamples');
        const minSpanHours = (0, params_1.num)(ctx.params, 'minSpanHours');
        const unit = typeof ctx.params.unit === 'string' ? ctx.params.unit : '';
        if (maxRate <= 0)
            throw new Error('maxRatePerHour must be positive');
        if (minSpanHours <= 0)
            throw new Error('minSpanHours must be positive');
        ...
        for (const s of stats) {
            ...
            findings.push({
                ...
                summary: `${name} ${s.matchedPath} consuming ${(0, params_1.round)(rate, 1)}${unit}/h ` +
                    `(${(0, params_1.round)(advance, 1)}${unit} over ${(0, params_1.round)(spanHours, 1)}h, limit ${maxRate}${unit}/h)`,
                detail: {
                    measurement: s.matchedPath,
                    ratePerHour: (0, params_1.round)(rate, 2),
                    maxRatePerHour: maxRate,
                    advance: (0, params_1.round)(advance, 2),
                    unit: unit || null,
                    firstValue: (0, params_1.round)(s.first, 2),
                    lastValue: (0, params_1.round)(s.last, 2),
                    spanHours: (0, params_1.round)(spanHours, 2),
                    samples: s.samples,
...
//# sourceMappingURL=counter-spike.js.map