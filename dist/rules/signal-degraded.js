...
const rule = {
    id: 'signal-degraded',
    description: 'Flags devices whose average best-gateway RSSI (or SNR) over the window is at or ' +
    ...
    async run(ctx) {
        ...
        const rssiThreshold = (0, params_1.num)(ctx.params, 'rssiThresholdDbm');
        const minUplinks = (0, params_1.int)(ctx.params, 'minUplinks');
        const snrRaw = ctx.params.snrThresholdDb;
        const snrThreshold = snrRaw === null || snrRaw === undefined ? null : (0, params_1.num)(ctx.params, 'snrThresholdDb');
        ...
        const rows = await ctx.query(`WITH per_uplink AS (
         SELECT dev_eui,
                device_name,
       ...
       HAVING count(*) >= $2::int
        ...
        ORDER BY avg(best_rssi) ASC`, [lookbackHours, minUplinks, rssiThreshold, snrThreshold, ...sc.values]);
        return rows.map((row) => {
            ...
            return {
                ...
                summary: `${name} link degraded: avg best-gateway RSSI ${avgRssi}dBm${snrPart} ` +
...
//# sourceMappingURL=signal-degraded.js.map