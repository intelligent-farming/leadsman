...
const rule = {
    id: 'device-silent',
    description: 'Flags devices that reported recently enough to be considered in service but ' +
    ...
    async run(ctx) {
        const silentMinutes = (0, params_1.int)(ctx.params, 'silentMinutes');
        const inventoryHours = (0, params_1.int)(ctx.params, 'inventoryHours');
       ...
       HAVING count(*) >= $2::int
        ...
        ORDER BY max(time) ASC`, [inventoryHours, minUplinks, silentMinutes, ...sc.values]);
        return rows.map((row) => {
            const minutes = Number(row.silent_minutes);
            const label = row.device_name ?? row.dev_eui;
            const forText = minutes >= 1440
                ? `${(0, params_1.round)(minutes / 1440, 1)}d`
                : minutes >= 60
                    ? `${(0, params_1.round)(minutes / 60, 1)}h`
                    : `${minutes}m`;
            return {
                devEui: row.dev_eui,
...
//# sourceMappingURL=device-silent.js.map