...
const rule = {
    id: 'device-log-error',
    description: "Flags devices ChirpStack has logged errors about in event_log — codec failures, " +
    ...
    async run(ctx) {
        ...
        const minLevel = (0, params_1.int)(ctx.params, 'minLevel');
        const minEvents = (0, params_1.int)(ctx.params, 'minEvents');
        ...
        if (minLevel < 0)
            throw new Error('minLevel must not be negative');
        ...
        GROUP BY dev_eui
       HAVING count(*) >= $3::int
        ORDER BY max(level::int) DESC, count(*) DESC`, [lookbackHours, minLevel, minEvents, excludeCodes, ...sc.values]);
        return rows.map((row) => {
            ...
            return {
                ...
                summary: `${name}: ${events} ChirpStack ${noun}${events === 1 ? '' : 's'} in ` +
                ...
                detail: {
                    ...
                    eventsPerHour: (0, params_1.round)(events / lookbackHours, 2),
                },
            };
        });
    },
...
//# sourceMappingURL=device-log-error.js.map