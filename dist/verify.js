 ...
 */
async function verify(config, rules, store) {
    const problems = [];
    ...
    if (!schemaPresent) {
        problems.push({
            severity: 'error',
            where: 'database',
            message: 'leadsman.alert not found — apply migrations/001_leadsman_schema.sql ' +
    ...
    if (live.size === 0) {
        problems.push({
            severity: 'error',
            where: 'database',
            message: 'no tables visible in schema "public" — either ChirpStack has not written ' +
        ...
        for (const key of Object.keys(check.params ?? {})) {
            if (!(key in rule.defaultParams)) {
                problems.push({
                    ...
                    message: `"${key}" is not a parameter of rule "${rule.id}" and will be ignored ` +
            ...
            for (const col of req.columns) {
                ...
                if (!cols.has(col) && !reported.has(colKey)) {
                    ...
                    problems.push({
                        severity: 'error',
                        where: `rule.${rule.id}`,
                        message: `column public.${req.table}.${col} does not exist — this ChirpStack ` +
